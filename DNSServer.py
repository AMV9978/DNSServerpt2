...
# IMPORTANT: do not modify/decode/decrypt the package you exfiltrate in TXT
...
# Helper to ensure TXT is a single quoted chunk so the Fernet token isn't mangled
def _quote_txt_if_needed(s: str) -> str:
...
dns_records = {
    ...
    # Required records per assignment
    ...
    'nyu.edu.': {
        ...
        # Store a STRING-CAST version of your encrypted data (do NOT decrypt)
...
def run_dns_server():
    # Create a UDP socket and bind it to the local IP address (127.0.0.1) and port 53 (standard DNS)
        ...
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    ...
                    mname, rname, serial, refresh, retry, expire, minimum = answer_data # What is the record format? See dns_records dictionary.
                    ...
                    token = answer_data[0] if isinstance(answer_data, (list, tuple)) else str(answer_data)
                    token = _quote_txt_if_needed(token)
                    rdata_list = [dns.rdata.from_text(dns.rdataclass.IN, qtype, token)]
                else:
