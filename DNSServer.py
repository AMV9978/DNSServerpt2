...
# For TXT we keep the raw BYTES so we can construct TXT RDATA without any quoting.
dns_records = {
    'example.com.': {
        ...
        dns.rdatatype.TXT: (b'This is a TXT record',),
        dns.rdatatype.SOA: (
    ...
    'nyu.edu.': {
        ...
        # Store the ENCRYPTED BYTES directly for TXT so we can set TXT(strings=(bytes,))
        ...
        try:
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    ...
                    # Build TXT with a SINGLE bytes chunk; no quoting involved anywhere.
                    payload = answer_data[0] if isinstance(answer_data, (tuple, list)) else answer_data
                    if isinstance(payload, str):
                        payload = payload.encode('utf-8')
                    rdata_list.append(TXT(dns.rdataclass.IN, dns.rdatatype.TXT, (payload,)))
                else:
            ...
            # Keep running on malformed queries
            try:
